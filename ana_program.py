        ...
        with tab_arsiv:
            ...
            if isinstance(arsiv_tarih, tuple) and len(arsiv_tarih) == 2:
                ...
                if not df_arsiv_isler.empty:
                    df_arsiv_isler['id_gosterim'] = df_arsiv_isler.apply(lambda r: f"🟢 {r['id']}" if r.get('Faturali_Mi', r.get('faturali_mi', 1)) == 0 else f"🔴 {r['id']}", axis=1)
...
# 🚨 GEÇMİŞ VERİLERİ SENKRONİZE ET BUTONU (BİR KERE KULLANILACAK) 🚨
        
with st.expander("🛠️ Geçmiş Sarfiyatları Makinelere İşle (Senkronizasyon)", expanded=False):
        
    st.warning("Bu işlem, geçmişteki tüm reçetelerde (CAM ve Sinter) harcanan dakikaları tarayacak ve ilgili makinelerin güncel çalışma saatlerinin üzerine ekleyecektir. **Daha önce eklendiyse mükerrer ekleme yapabilir, lütfen sadece 1 kez kullanın!**")
        
    if st.button("Geçmiş Sarfiyatları Senkronize Et", type="primary"):
        
        try:
        
            import json, re
        
            isler = c.execute("SELECT id, Harcanan_Malzeme, Sinter_Sarfiyati FROM isler").fetchall()
            ...
            for r in isler:
                ...
                if cam and "Makine:" in cam:
        
                    match = re.search(r'Makine:\s*(.*?),\s*Frezler.*?Top\.\s*(\d+)\s*Dk', cam)
        
                    if match:
        
                        makine = match.group(1).strip()
        
                        dk = int(match.group(2))
        
                        machine_times[makine] = machine_times.get(makine, 0) + dk
        
                if sinter and sinter != "-" and sinter.startswith("{"):
        
                    try:
        
                        sdata = json.loads(sinter)
        
                        f1 = sdata.get("f1", "-- Seçiniz --")
        
                        s1 = sdata.get("s1", 0)
        
                        f2 = sdata.get("f2", "-- Seçiniz --")
        
                        s2 = sdata.get("s2", 0)
        
                        if f1 != "-- Seçiniz --" and s1 > 0: machine_times[f1] = machine_times.get(f1, 0) + s1
        
                        if f2 != "-- Seçiniz --" and s2 > 0: machine_times[f2] = machine_times.get(f2, 0) + s2
        
                    except: pass
        
            for makine, dk in machine_times.items():
        
                if makine and makine != "-- Seçiniz --":
        
                    c.execute("UPDATE cihazlar SET Calisma_Saati = Calisma_Saati + ? WHERE Cihaz_Adi = ?", (dk / 60.0, makine))
        
            conn.commit()
        
            st.success(f"Başarılı! Toplam hesaplanan ek dakikalar: {machine_times}")
        except Exception as e:
            st.error(f"Hata: {e}")
