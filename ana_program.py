...
if not st.session_state["giris_yapildi"]:
    st.markdown("""<style>
/* ── DENTMESHER.COM GÖRSEL KİMLİĞİ (CANLI ANİMASYONLU ÇOK KOYU LACİVERT) ── */
...
/* ── HAREKETLİ IŞIMA VE AURA KÜRELERİ ── */
.dm-aurora-wrap {
    position: fixed;
    top: 0;
    left: 0;
...
.dm-orb {
    ...
    pointer-events: none;
    opacity: 0.55;
    animation: dmFloatOrb 18s infinite alternate ease-in-out;
}
...
@keyframes dmFloatOrb {
    ...
    33% { transform: translate(60px, -45px) scale(1.10) rotate(35deg); }
    66% { transform: translate(-45px, 55px) scale(0.95) rotate(-25deg); }
    100% { transform: translate(35px, -35px) scale(1.05) rotate(15deg); }
}

.dm-particle {
    position: absolute;
    border-radius: 50%;
    background: rgba(232, 98, 44, 0.65);
...
<div class="dm-particle-blue" style="width:6px;height:6px;left:28%;animation-duration:18s;animation-delay:-3s;"></div>
<div class="dm-particle" style="width:5px;height:5px;left:45%;animation-duration:16s;animation-delay:-7s;"></div>
<div class="dm-particle-blue" style="width:4px;height:4px;left:68%;animation-duration:20s;animation-delay:-2s;"></div>
<div class="dm-particle" style="width:7px;height:7px;left:82%;animation-duration:15s;animation-delay:-9s;"></div>
<div class="dm-particle-blue" style="width:5px;height:5px;left:93%;animation-duration:22s;animation-delay:-5s;"></div>
...
/* ── ŞEFFAF, ÇERÇEVESİZ, DİKEY VE SAĞA YASLI BUTONLAR ── */
...
# --- 1. SEVİYE: SAĞA YASLANMIŞ EŞİT BOYUTLU ANA BANNER (ST.BUTTON İLE %100 GÜVENLİ VE ANLIK) ---
num_cats = len(gecerli_kategoriler)
col_weights = [2.2, 3.2] + [1.0] * num_cats
...
# Sol: Logo
with kat_cols[c_idx]:
    st.markdown("<div style='font-family:Manrope,sans-serif; font-weight:900; font-size:18px; color:#f8fafc; padding-top:14px; white-space:nowrap;'>DENTMESHER <span style='color:#e8622c;'>HUB</span></div>", unsafe_allow_html=True)
c_idx += 1

# Orta: Sağa yaslama boşluğu
with kat_cols[c_idx]:
    st.write("")
c_idx += 1
...
# Banner Ayırıcı Çizgi
st.markdown("<div style='height:1px; width:100%; background:linear-gradient(90deg, rgba(56,189,248,0.25) 0%, rgba(255,255,255,0.08) 50%, rgba(56,189,248,0.25) 100%); margin:4px 0 12px 0;'></div>", unsafe_allow_html=True)
            ...
            if kat == "-- Seçiniz --":
                ...
                with st.form("yeni_stok_formu"):
                    ...
                    if kat in ["Blok", "Zirkonyum Blok"]:
                        ...
                        malzeme = c3.text_input("Malzeme", value=d_malzeme, key=f"malz_{sec_kod_display}")
                        toz_boyutu = c1.text_input("Toz Boyutu (Örn: 10µm)", value=d_toz_boyutu, key=f"toz_{sec_kod_display}")
                        alasim = c2.selectbox("Alaşım Türü", ["Nikelli", "Nikelsiz"], index=0 if d_alasim=="Nikelli" else 1, key=f"alasim_{sec_kod_display}")
                    ...
                    if not df_arsiv2.empty:
                        for _, ar_row in df_arsiv2.iterrows():
                            ...
                            with st.container(border=True):
                                ...
                                is_disabled = bool(fat_id_ar)
                                btn_help = "Ekstreyi iptal edip siler." if not is_disabled else "Ekstreye bağlı bir fatura var. Lütfen önce 'Faturalar' sekmesinden faturayı iptal edin."
                                if ar3.button("🗑️ Geri Çek", key=f"ar_geri_{ar_row['id']}", use_container_width=True, disabled=is_disabled, help=btn_help):
                                    if not is_disabled:
                                        try:
